...
interface
  ...
  type
   ...
   TDllFile = class
   // TODO : change attribute to private after end of debuging
   public
     //
   ...
   public
     ...
     function FindFunctionByName(name : string) : boolean;
   ...
   public
      ...
      ImportsArray : TFunctions;
      ImportTable: PImageDataDirectory;
      BoundImportTable: PImageDataDirectory;
      DelayImportTable: PImageDataDirectory;

      Donor : TDonorFunction;

      ITATable: PImageDataDirectory;

      function FindDOSHeaders : boolean;
      function GetSections : boolean;
      function GetImportTable : boolean;
      function GetBoundImportTable: Boolean;
      function GetITATable: Boolean;
      function GetDelayImportTable : boolean;
      function GetImportFunctionsFromTable : boolean;
      // function ConvertPointer(RVA : LONGWORD) : Pointer;


   public
      constructor Create;
      function FindReloadedFunction: Boolean;
      function GetAddressOfReloaderFunction(const ModuleName, FuncName: string):
          Boolean;

      function RVAtoReal(RVA : LONGWORD): pointer;

      function GetNewProcAddress(hModule: HMODULE; lpProcName: LPCSTR): FARPROC; stdcall;

   end;

implementation

function TDllFile.ConvertPointer(RVA: LONGWORD): POINTER;
var
  I : integer;
begin
  Result := nil;
...
constructor TDllFile.Create(mem : TMemoryStream);
begin
  inherited Create;
  ImageBase := nil;
  DLLProc := nil;
...
function TDllFile.FindExternalLibrary(LibraryName: string): integer;
var
  I : integer;
begin
  Result := -1;
...
function TDllFile.FindFunctionByName(name: string): boolean;
begin
  result := true;
end;

function TDllFile.GetExportList: TStringList;
var
  I: integer;
begin
...
function TDllFile.GetExternalLibraryHandle(LibraryName: string): HINST;
var
  I : integer;
begin
  Result := 0;
...
function TDllFile.InitializeImage: boolean;
var
  SectionBase: POINTER;
  OldPosition: integer;
  OldProtect: LONGWORD;
...
function TDllFile.InitializeLibrary: boolean;
begin
  Result := FALSE;
  SysErrorCode := 0;
...
function TDllFile.IsImportByOrdinal(ImportDescriptor: DWORD;
  HLib: THandle): boolean;
begin
    result:=(ImportDescriptor and IMAGE_ORDINAL_FLAG32)<>0;
end;

function TDllFile.LoadExternalLibrary(LibraryName: string): integer;
begin
...
function TDllFile.LoadFromStream : boolean;
begin
  Result := false;
...
function TDllFile.ParseStringToNumber(Astring: string): LONGWORD;
var
  CharCounter: integer;
begin
  Result := 0;
...
function TDllFile.ProcessExports: boolean;
var
  I: integer;
...
function TDllFile.ProcessImports : boolean;
var
...
function TDllFile.ProcessRelocations: boolean;
var
  Relocations: PCHAR;
  Position: LONGWORD;
...
function TDllFile.ProtectSections: boolean;
var
   I: integer;
...
function TDllFile.ReadImageHeaders: boolean;
begin
   Result := FALSE;
   if Stream.Size > 0 then
   begin
...
function TDllFile.ReadSections: boolean;
var
  I: integer;
...
function TDllFile.Unload: boolean;
var
  I, J: integer;
begin
  Result := FALSE;
...
constructor TReloader.Create();
var
  aHandle : HModule;
begin
  // First Get a handle of exe module
  aHandle  := GetModuleHandle(nil);
  // Check is a Correct handle
  if (aHandle <> INVALID_HANDLE_VALUE) or (aHandle <> 0) then
      handle := aHandle
  else
      raise Exception.Create('Error Handle for Main Program');

end;

function TReloader.FindDOSHeaders: boolean;
begin
   Result := FALSE;
   try
      DOSHeader := Pointer(handle);
      if DOSHeader.Signature <> $5A4D then Exit;
      NTHeader := Pointer(handle + DOSHeader.LFAoffset);
      if NTHeader.Signature <> $00004550 then Exit;
      if NTHeader.FileHeader.Machine <> $14C then Exit;
   except
      raise Exception.Create('Error when try to read Process headers ' + SysErrorMessage(GetLastError));
   end;

   Result := TRUE;
end;

function TReloader.FindReloadedFunction: Boolean;
var
  I: Integer;
begin
  Result := false;

  If Donor.Address = 0 then exit;

  for I := Low(ImportsArray) to High(ImportsArray) do
  begin
     if ImportsArray[I].FunctionAddress = Donor.Address then
     begin
        Donor.Index := I;
        Donor.Func_ := ImportsArray[I].FunctionPointer;
        Result := true;
        exit;
     end;
  end;
...
function TReloader.GetAddressOfReloaderFunction(const ModuleName, FuncName:
    string): Boolean;
var
  libHandle : THandle;
  ProcAddr :  Pointer;
...
begin
  ...
  if LibHandle <> 0 then
  begin
     ProcAddr := GetProcAddress(libHandle, PChar(FuncName));
     if ProcAddr <> nil then
     begin
        Donor.Address := Cardinal(ProcAddr);
        Donor.LibHandle := LibHandle;
        Donor.Name := FuncName;
        result := true;
     end;
  end;
...
function TReloader.GetBoundImportTable: Boolean;
begin
  Result := False ;
  if NTHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BOUND_IMPORT].VirtualAddress <> 0 then
  begin
    BoundImportTable := RVAtoReal(NTHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BOUND_IMPORT].VirtualAddress);
    result := true;
  end;
end;

function TReloader.GetDelayImportTable: boolean;
begin
  result := false;
  if NTHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DELAY_IMPORT].VirtualAddress <> 0 then
  begin
    DelayImportTable := RVAtoReal(NTHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DELAY_IMPORT].VirtualAddress);
    result := true;
  end;
end;

function TReloader.GetImportFunctionsFromTable: boolean;
var
  ImportDescriptor : PImageImportDescriptor;
  ImportCount: Integer;
  ThunkData: PImportThunkData;
...
begin
  ...
  while ImportDescriptor^.Name <> 0 do begin
     ...
     while ThunkData.ForwarderString <> 0 do begin
          ...
          ImportsArray[ImportCount-1].LibraryName := LibraryName;
          ImportsArray[ImportCount-1].FunctionAddress := ThunkData.Function_;
          // TODO :  If not work need skiip integer size
          ImportsArray[ImportCount-1].FunctionPointer := ThunkData;
          Inc(ThunkData);
     end;
  ...
  If Length(ImportsArray) > 0 then result := true;

end;

function TReloader.GetImportTable: boolean;
begin
  result := false;
  if NTHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress <> 0 then
  begin
    ImportTable := RVAtoReal(NTHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress);
    result := true;
  end;
end;

function TReloader.GetITATable: Boolean;
begin
  Result := false;
  if NTHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IAT].VirtualAddress <> 0 then
  begin
    ITATable := RVAtoReal(NTHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IAT].VirtualAddress);
    result := true;
  end;
end;

function TReloader.GetNewProcAddress(hModule: HMODULE;
...
begin
  ...
  if (lpProcName = 'Start' ) then
  begin
    TempHandle := LoadLibrary('testDLLReload.dll');
...
function TReloader.GetSections: boolean;
begin
  Result := FALSE;
  if NTHeader.FileHeader.NumberOfSections > 0 then
  begin
     Sections := RVAtoReal(DOSHeader.LFAoffset + Sizeof(TImageNTHeaders));
     Result := TRUE;
  end;
end;

function TReloader.RVAtoReal(RVA : LONGWORD): pointer;
begin
